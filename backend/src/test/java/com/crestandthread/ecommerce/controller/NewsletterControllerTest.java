...
class NewsletterControllerTest {
    ...
    class CheckSubscription {
        ...
        void shouldReturnFalseWhenEmailIsNotSubscribed() throws Exception {
            // Given
            when(newsletterService.isSubscribed("notsubscribed@example.com")).thenReturn(false);

            // When/Then
            mockMvc.perform(get("/api/v1/newsletter/check")
                            .param("email", "notsubscribed@example.com"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.success").value(true))
